...
The immutable candidate commit declares `source.phase = source-candidate`, version `2.0.0-rc.6`, latest published tag `v2.0.0-rc.5`, and rollback target `v2.0.0-rc.5`. It records no completed delivery action. After exact-SHA delivery succeeds, a separate current-truth commit advances the published identity to RC.6, preserves RC.5 as rollback, and records only observed external results.
