...
One full qualification run contains all three complete attempts. An infrastructure failure preserves its first evidence and triggers a candidate/runner decision before any new full run; it is never locally retried unchanged to manufacture completion.
