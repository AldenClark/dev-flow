...
S8 remains active on independent-review corrections and the contradictory macOS 15/Python 3.14 evidence. Failed and passing runs used the same macOS 15.7.7 arm64 image, Python 3.14.6, runner version, product lease bytes, synthetic input, and isolated temporary-root design; deterministic version drift and shared-directory contamination are therefore rejected hypotheses. The final auxiliary diagnostic completed without recurrence, including 25/25 instrumented black-box CLI round trips on the target cell. The independent review then exposed three candidate-integrity/lifecycle defects; their local repairs are under final deterministic, hosted Windows, and independent recheck. Live three-attempt R4 execution and final hosted artifacts have not run.

## Next

Complete the identity/allowlist/Windows-lineage repair verification, push its exact bytes through the full hosted matrix, and obtain the existing independent reviewer's focused recheck. Retain HC4 as `flaky` until the version owner explicitly waives the low-frequency hosted residual risk. Start the already-authorized R4 execution only after S8/HC4 and final-byte review close. Do not dispatch hosted artifacts, tag, publish, or install while these prerequisite gates remain open.
...
| HC4 | Resource lease/preflight behavior passes the supported Ubuntu/macOS/Windows matrix or unsupported cells remain release-blocking | implementation | flaky | `e808c1a` failed macOS 15/Python 3.14; `75150a3` passed the complete matrix and `14aa583` passed it again plus 25/25 target-cell diagnostics without a product lease change; explicit owner waiver remains required |
