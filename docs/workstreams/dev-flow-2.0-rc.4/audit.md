...
- Windows execution now creates a kill-on-close Job Object before spawning each bounded subprocess, assigns the direct child immediately, closes the job on success, timeout, creation/communication failure, and falls back fail-closed if assignment fails. Mocked lifecycle tests cover success, timeout, and launch failure; a Windows-only integration starts a descendant from a successful parent and requires that descendant to terminate after the job closes. Both runner source and its test now force the hosted compatibility matrix.
