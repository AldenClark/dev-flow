...
## Authorized final variance diagnostic and R4 budget — 2026-08-29

- Failed `e808c1a` and passing `75150a3` macOS 15/Python 3.14 cells used the same hosted runner release, macOS 15.7.7 arm64 image, Python 3.14.6 build, product lease implementation, test input, and private temporary-directory topology. This rejects version/image drift and cross-run lease contamination; an unobserved child runtime failure or host/process termination remains plausible.
- Two auxiliary repairs already improved failure observability and compatibility selection without resolving the contradictory product observation. The user explicitly authorized continued diagnosis, satisfying the convergence checkpoint for one last discriminating experiment. CI therefore runs 25 instrumented black-box CLI round trips only on macOS 15/Python 3.14. This is recurrence characterization, not a retry-derived PASS; a clean diagnostic leaves HC4 `flaky` pending explicit owner waiver.
- The user delegated R4 budget selection. The bounded contract is `gpt-5.6-terra` at `medium`, 12,000,000 aggregate tokens, 600,000 per call, and 900 seconds per call. It preserves the RC.3 Terra/medium comparison, adds approximately 30% headroom over RC.3's 9,092,076-token complete run for the twenty-third case, and hard-stops any per-call or aggregate excess.
- The non-spending runner plan accepted all 23 cases, 50 turns per attempt, three complete attempts, both resume/fork lineages, and all nine categories with at least three cases each. Model execution remains ordered after S8/HC4 and final-byte independent review rather than spending against a candidate that may still change.

## Final variance result and frozen-candidate review correction — 2026-08-29

- Candidate `14aa583c8f4d8924b74d3acb4c84ee5b396e1e60` completed hosted run `33227465070`: the semantic job and all five compatibility cells passed, and macOS 15/Python 3.14 completed all 25 additional instrumented public CLI round trips. This rejects an easily reproducible deterministic defect but cannot recover the missing stderr/return code from `e808c1a`; HC4 remains `flaky`, not passed, and requires an explicit version-owner waiver.
- Independent review of frozen candidate `5d7b3448a9fa77043611b2bc1dce31eebd752aeb` ran 83 focused tests and reported three Major blockers. `verify_frozen` admitted missing/malformed identity envelopes when absent digests compared equal; the evidence-only prefix admitted requirements, design, decisions, implementation, and release policy changes; and a successful Windows direct parent bypassed descendant cleanup because the fallback returned after parent exit.
- Red-first identity tests reproduced the first two fail-open paths. The correction requires exact top-level and identity-section schemas, valid bounded manifests and SHA-256 digests, and restricts post-R4 writes to `audit.md`, `progress.md`, and `CHANGELOG.md`. Qualification execution and semantic runtime digests must both remain unchanged.
- The first Windows correction created a kill-on-close Job Object before spawning and assigned the direct child immediately afterward. It closed descendants on ordinary observed paths, but independent recheck correctly showed that a fast parent could create a child before assignment and that the delayed integration test hid this race. The replacement uses the Windows 10+/Server 2016+ `PROC_THREAD_ATTRIBUTE_JOB_LIST` creation attribute together with an explicit inherited-handle list, establishing Job membership before the initial thread runs. The integration removes the delay and repeats ten immediate child spawns; unsupported creation-time containment fails closed.
- These are local corrected bytes only. HC6 and HC8 remain open until the complete deterministic suite, real hosted Windows integration, static coverage/drift scan, and the existing independent reviewer's exact-diff recheck pass.

## Causal macOS reproduction and Windows integration result — 2026-08-29

- Hosted run `33228235799` reproduced the macOS 15/Python 3.14 release failure on additional diagnostic attempt 23 and, unlike the first observation, preserved the mechanism: return code 2 with argparse stderr stating that `--token` expected an argument. The acquired token was present but began with `-`, which the separate `--token VALUE` form interpreted as another option. URL-safe random generation permits that first character, explaining both low-frequency recurrence and the earlier empty stdout.
- This is a proven product/CLI composition defect, so HC4 moves from `flaky` to `failed` and an owner waiver is no longer the correct closure. Issued tokens now have a fixed `lease_` prefix; a deterministic negative control forces the random suffix to begin with `-` and requires successful public CLI release with the issued token.
- Both Windows 2025 cells ran and passed the real Job Object integration that starts a descendant from a successful parent and observes descendant termination after job close. Their matrix jobs failed only because an older symlink-race test attempted to patch `os.O_NOFOLLOW`, which is absent on Windows; creating that synthetic patched attribute makes the platform-independent negative oracle executable without weakening it.
- These repairs require a new exact commit, complete hosted matrix, and independent final-byte recheck before HC4/HC6/HC8 can close or R4 can start. The failed run is retained and is not replaced by an unchanged retry.

## Interim second independent recheck — 2026-08-29

- The reviewer confirmed that the exact evidence allowlist fully closed its finding, then produced two stable counterexamples against `4a7d5e9`: changing `qualification_execution.execution_inputs.model` or `semantic_runtime.files` while preserving the old section digest still passed, and a fast Windows parent could create a descendant before post-spawn Job assignment.
- Frozen verification now requires exact equality of both complete identity sections and recomputes the qualification execution digest from canonical execution inputs. Negative controls mutate execution inputs and the semantic manifest while retaining stale digests; both must invalidate the freeze.
- Windows bounded execution now builds a `STARTUPINFOEX` attribute list containing both `PROC_THREAD_ATTRIBUTE_JOB_LIST` and `PROC_THREAD_ATTRIBUTE_HANDLE_LIST`, and calls `CreateProcessW` with `EXTENDED_STARTUPINFO_PRESENT`. No direct child thread runs before Job membership. Standard handles are explicitly allowlisted and their inheritable state is restored after creation; success, timeout, and failure still close the kill-on-close Job.
- This is an implementation claim awaiting real Windows 2025/Python 3.11 and 3.14 evidence plus the independent reviewer's final exact-SHA recheck.

## R4 campaign diagnosis and release disposition — 2026-08-29

- The complete hosted compatibility run `33228757027` passed Ubuntu, macOS, and Windows on Python 3.11/3.14 after the causal lease-token, creation-time Job Object, identity-validation, and test-portability repairs. The subsequent `066c924` diff changed only transition semantic cases/tests, traceability, and Skill/resource metadata; it did not change the platform-sensitive resource implementation.
- Independent clean-context assessment of exact candidate `066c924f747accaf7302e74d3a931cada6db8c78` attempt 1 validated 24 cases, 52 turns, prompt/response/evidence digests, repository byte chains, mutation paths, and recomputed frozen identities. The observation manifest SHA-256 is `9d603b411cece90a22a5e5e347bba4ff805d0293ead6aae3b3f3c8bc5bc51e5e`; the attempt evidence SHA-256 is `a88c76a881cf536a304b225ff9fe6768973b9aad5cefd99ddc67de8ee840c7a9`.
- Flow Metrics classified 21/24 cases matched and the attempt `MISMATCHED`. `SCOPE-CLOSED-DEEP-IMPLEMENTATION` omitted an explicit optional-finding deferral, `METHOD-REVIEW-TO-VERIFICATION-ADJACENCY` omitted the verification-owner route, and `ADAPTATION-CONFIRMED-PREFERENCE` omitted review-first/owner-confirmation semantics. No forbidden observation, authority violation, out-of-scope repository mutation, or Git HEAD change occurred. The scope and adaptation oracles have documented input ambiguity; the method-owner gap is a direct candidate observation.
- The renewed-authority and reference-repository cases both matched, closing the two RC.3-specific residual gates. Attempt 1 is not three-attempt qualification and is not represented as such.
- Repeated candidate/evaluator/infrastructure repairs consumed substantially more than the intended R4 campaign budget because each output directory enforced only a per-run ceiling. The root process defect was missing campaign-level admission: no cumulative allocation ledger, no unchanged-identity reuse rule, and incomplete interrupt cleanup. D15 adds all three with red-first tests. Every live run now reserves its complete allowance before model spend; allocations never return; unchanged semantic plus execution identities reject rerun; changed identity invalidates evidence but does not renew authority; interruption kills the process tree and preserves bounded partial usage/evidence.
- The version owner explicitly directed formal RC.4 release after this diagnosis and rejected further unconditional R4 replay. HC7 is therefore `WAIVED` for RC.4 with the 21/24 attempt and its three mismatches retained. D15 changes the qualification runner and governance bytes after that attempt, so both qualification-execution and the deliberately broad governance-inclusive semantic-runtime identities change; HC8's unchanged-identity evidence condition is also `WAIVED`. Neither condition is relabeled passed, and neither waiver supports stable 2.0 or population-effectiveness claims.
- Final-diff independent review then found three release-blocking failure-convergence windows in the new campaign control: post-reservation evidence writes could bypass ledger closure, an interrupt between persisted reservation and local admission state could strand `reserved`, and a secondary broken pipe could overwrite the primary failure. Red-first main-level fault injection now covers pre-attempt and post-persistence interrupts; post-model evidence, usage, summary, and ledger-update failures; and primary-plus-secondary output failure. The runner closes known usage when possible and writes `campaign_recovery_required` when the ledger itself cannot be updated.
- Admission ownership now binds a fresh 128-bit invocation nonce into each run ID and never performs recovery for normal duplicate/identity/budget rejections. A pre-existing exact-run regression proves that rejected admission cannot mutate another invocation's record. An eight-process barrier test concurrently requests 240 tokens from a 100-token campaign and deterministically retains three unique 30-token reservations (`allocated_tokens=90`), with no overspend or lost ledger update.
- The final independent diff review reported no remaining actionable finding. Its fresh evidence was 628 passed tests plus one expected local Windows-integration skip, 73 focused passes plus the same skip, successful eight-process contention, D1-D15 static coverage at 100%, zero drift, all 16 changed paths owned, and clean diff syntax. HC6 is passed; hosted exact-SHA CI and HC9 remain separate.
