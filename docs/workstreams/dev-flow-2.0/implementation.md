...
| S23 RC.2 activation hardening | Improve implicit kernel discovery, specialist reconnect, explicit/material routing, method normalization/ranking, independent-review truth, and managed interruption recovery without expanding the Skill or method inventory | active Skills/references, task router/method projection, workstream contracts, activation catalogs/tests, version truth | focused regressions, full deterministic suite, suite/plugin validation, final diff review; model-semantic trials separately budgeted | complete |
| S24 RC.2 audit repair and large-task simulation | Make invalid-input recovery portable and semantics-preserving, keep partial signals from suppressing risk families, expose weak-oracle/model-evaluation methods, and freeze representative multi-boundary workflows as executable simulations | task router/method projection, methodology guidance, large-task catalog/tests, evaluation docs, RC truth | correction replay from foreign CWD, combined-invalid recovery, method reachability, eight simulations plus oracle mutation, full deterministic suite and final review | complete |
| S25 RC.2 task-family coverage expansion | Expand large-task routing from a small regression sample to balanced common-work coverage, add distribution gates and multiple oracle mutations, and repair any route gap the expanded sample exposes | 80-case catalog, coverage tests, capability registry, evaluation docs, RC truth | 20 families × 4 variants, six intents, five requirement classes, direct/managed and all overlays/owners, negative controls, focused/full deterministic verification | complete |
