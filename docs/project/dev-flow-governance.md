...
After repository context, the Assurance Method System maps explicit lifecycle phase, task type, risks, observed failure signals, and evidenced prerequisites through 38 deterministic risk models. Its canonical pool contains 117 source-bound methods across 10 phases and 16 families, with one lightweight foundation per phase plus risk-triggered `starter`, `deep`, or `formal` methods. AI coding and general-agent coverage includes autonomy selection, partial observability, receding-horizon/reactive/hierarchical planning, counterexample-guided repair, runtime shields and temporal monitors, compensation, provenance/memory governance, human function allocation, multi-agent topology/dynamic allocation, trajectory intervention evaluation, calibrated selective action, and simulation. Design/review coverage also includes architecture viewpoint consistency, source-to-architecture reflexion/conformance, BPMN collaboration workflows, data-quality reconciliation, cost-benefit architecture analysis, and cross-language ABI/ownership contracts. Specialist failure models are split from broad stacks so a broad task/risk or `formal` depth cannot activate them without their specific signals. Selection is capped, stable, and progressive; negative triggers prevent cargo-cult escalation, missing prerequisites remain unresolved with fallbacks, and every result preserves the existing owner Skill and authority boundary. The selected method is guidance for producing an artifact and failure-sensitive evidence, never proof that the method or an external tool was executed.

All 55 engineering-context risks are covered directly or by 23 explicit methodology aliases; routing reports canonical translations and unmapped risks. New governed packets advertise immutable `method-selection-v1`, create a preliminary design selection, and require fresh non-preliminary design, verification, and review records at their respective lifecycle gates. `method-selection.json` and its deterministic Markdown rendering are event-projected and bound to the registry, requirement/design bytes, current packet risks, selected owner, and planned packet artifact. Legacy packets retain their creation contract and are not silently upgraded.
