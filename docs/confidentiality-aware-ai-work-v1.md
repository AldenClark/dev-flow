...
- **阻断**：仅高置信 C4 进入 Codex 待发送提示词/工具参数，或明显凭据将被写入外部目的地。
...
仓库实施已经完成规则/样例、共享 Skill、Codex 薄 Hook、本地 scan/redact/pseudonymize、三表面模板、doctor 和合成红蓝测试。V1 没有实现可逆 Token Vault、权限 profile 自动改写、MCP 自动收紧或集中式连接器策略。
...
主要残余风险：人工配置可被修改、完整性基线不是签名且可被有意协同篡改、Skill 可能未触发、普通 Chat 没有统一发送前拦截、云端任务无法依赖本机工具、连接器权限可能过宽、分隔/加密/压缩/低熵或新格式秘密可能漏检、直接标识符可能误报、产品能力会随版本变化。以上风险由自测、版本清单、最小权限、合成测试和定期复核降低，但不会被描述为消除。
