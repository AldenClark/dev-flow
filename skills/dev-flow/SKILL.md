...
- An optional scanner, reviewer, browser, or specialist request does not authorize discovery, install, or an external call. Call only an exact host-exposed tool or repository-native command; a capability label is not identity. Never invent MCP server/tool names or treat unrelated local MCP as authorized. If the exact capability is absent or unauthorized, preserve the gate, continue safe native checks, and do not substitute Web, browser, MCP, app, computer-use, image-generation, or dynamic tools. An explicitly named runner/user-owned tool remains limited to its exact authorization.
