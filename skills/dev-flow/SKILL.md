...
- Authority first: implementation does not imply commit, push, publish, deploy, migrate, install, external communication, model spend, or destructive cleanup. Child proposals grant no scope; exact renewed authority is required.
...
- A request for an optional scanner, reviewer, browser, or specialist does not authorize discovering, installing, or invoking an external capability. When no exact callable capability or repository-native command is already available, preserve the unavailable gate, continue only safe native checks, and do not call Web, browser, MCP, app, computer-use, image-generation, or dynamic tools to replace it.
