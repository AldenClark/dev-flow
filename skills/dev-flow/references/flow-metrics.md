...
Do not optimize a single proxy. A token reduction that increases missed defects, repair loops, or unverified claims is a regression. Use deployment-frequency/lead-time/change-failure/recovery measures only for real delivery systems, not for a local code-edit task.
