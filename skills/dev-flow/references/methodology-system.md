...
Task-facing guidance ranks directly matching ready methods separately from blocked fallbacks. It exposes at most three ready methods and two relevant blocked methods, so a missing prerequisite cannot consume the ready-guidance budget. When change/implementation has no matching method but the observed signal belongs to requirements, diagnosis, design, or verification, the route selects that adjacent owner phase instead of returning an empty implementation result. A valid actionable result contains ready steps or an exact blocked prerequisite and fallback; otherwise it reports `no-actionable-match` and returns control to the owning specialist. Privacy, persistent-agent-memory, and multi-agent coordination methods remain gated by observed privacy data, an agentic memory store, or actual delegation respectively.
