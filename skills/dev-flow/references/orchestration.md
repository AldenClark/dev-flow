...
When independent review has decision value, dispatch a clean-context reviewer through `route-agent` when the host supports it. Spawning the reviewer needs no separate authorization. If a real independent context is unavailable, report the capability downgrade, use the strongest same-context review available, and retain `common-mode-risk` as an evidence limit. Blue/red lenses in the implementer's context do not satisfy independence.
