...
Use the returned child role, model, reasoning effort, and fork request. P0–P3 use GPT-6 Luna for closed work, including clear multi-step work at xhigh. P4–P5 use GPT-6 Sol for ordinary judgment through conflicting causes and careful review. P6 uses GPT-6 Astra only for compound, deep unresolved reasoning; risk or critical acceptance alone raises verification depth, not the model tier. PX remains an explicit exceptional campaign. For an interactive root task with unclear shape, recommend Sol medium without pretending this child router can switch the root model. Check model/effort against the actual host and dispatch only when both `delegate` and `dispatch_ready` are true; an unchecked route is advice, and a capability limit cannot silently use GPT-5.6.
