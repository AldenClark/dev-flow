...
Use this reference for a new dependency, material capability expansion, major/high-impact update, or removal with non-obvious consumers. The goal is a sound dependency decision and reversible implementation, not a second approval ledger.
...
1. Can the standard library or platform facility meet the need?
2. Can an existing dependency do so without harmful feature expansion?
3. Is a bounded local implementation simpler and safe over its lifetime?
4. Which maintained external candidate best fits the exact API and constraints?
...
Check only decision-relevant facts: current version/release status, maintenance, advisories, license, platform/toolchain support, selected features, build scripts/native code, transitive graph, runtime cost, migration, rollback, and removal. Use current primary sources for volatile claims and record a durable decision only when future maintainers need the rationale.
...
- Preserve a practical rollback path for high-impact changes.
