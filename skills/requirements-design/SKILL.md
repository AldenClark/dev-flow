...
6. For U1, publish the complete requirement understanding before technical design. Stop for confirmation only when a surviving user-owned choice would materially change behavior, contract, scope, or recovery, or when the user asks to review the understanding first. A confirmed plan or explicit implementation request with settled semantics can proceed; neither U1 classification nor an agent's wish for reassurance creates a stop. A correction replaces the affected understanding; ask again only if a material choice remains. Keep the class U1 after confirmation.
...
- Do not start technical design or product-code mutation while a material user-owned choice remains unresolved.
