...
8. Write confirmed material semantics, examples/anti-examples, protected behavior, and unresolved user choices back to the existing product or contract owner so technical design can read them. If no owner exists, ask `repository-knowledge` to choose the smallest discoverable owner; do not leave material semantics only in chat. A change record is justified only when reliable continuation crosses sessions, owners, or independent slices and existing owners cannot carry the navigation.
