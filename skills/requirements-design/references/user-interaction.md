...
Before invocation, record the affected `AMB-n`, `AC-n`, `SC-*`, `VO-n`, dependency, waiver, or delivery boundary, current revision/digest, and blocked work. After a response, accept exactly one presented non-empty option, or one non-empty free-form value only when Other was enabled. Reject unknown IDs, empty/conflicting/out-of-range values, and stale-revision answers. Persist the semantic disposition, actor, time, scope, affected IDs, next requirement revision, and tasks released; retain raw wording only when necessary and safe, never secrets or unnecessary personal payloads.
