...
Record only the identities that the action needs: exact source commit/tree and version; relevant dependency/configuration inputs; artifact name, checksum/provenance/signature where applicable; target environment and recipient; and the evidence run against those final bytes. For an installed, deployed, external, or production outcome, follow only applicable links from tested bytes to artifact, actual target/install, effective config/data, intended user flow or external effect, observation, and safe recovery/rollback. Compare the target's observed artifact and effective state with the intended inputs; a successful build or install command cannot resolve a wrong artifact or stale configuration. Re-read the final diff and distinguish fresh pass, fail, flaky, blocked, waived, and `NOT RUN` evidence.

Then check the relevant compatibility direction and operational path: reader/writer or client/server versions, data/ordering/resume behavior, platform and consumer compatibility, observation signal and owner, stop trigger, restore/rollback trigger, executable recovery path, cleanup, and residual limits. Name what observation would detect a bad rollout and who can act on it. Do not run a maximum checklist for an irrelevant tier, but do not call a recovery plan real until its trigger and path are concrete enough to execute.

State the narrowest supported status: `implemented`, `verified`, `accepted`, `release-ready`, or `delivered`. Perform only the authorized action, then verify its actual external result. A host-green/device-unknown result cannot claim device success; wrong-artifact or stale-config evidence blocks the affected target claim. Keep an observed failure as failed, an unavailable prerequisite as `BLOCKED`, and an unexecuted check as `NOT RUN`; unobserved CI, signing, target device, deployment, distribution, and production evidence remain `NOT RUN`.
